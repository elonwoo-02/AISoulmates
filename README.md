...
- 👤 账号体系：注册、登录、登出、JWT 刷新、获取当前用户信息
- 🖼️ 个人资料：头像/背景图上传与编辑
- 🔑 AI 配置：登录用户保存个人 API Key 与 Base URL
- 🎭 角色系统：创建、列表、详情、更新、删除
- 🏠 首页：角色流展示与搜索
- 👥 好友系统：创建/获取、列表、删除
- 💬 消息：聊天历史拉取与展示
- 🤖 AI 对话：基于 SSE 的流式回复
- 📚 知识库：LangGraph + LanceDB 的检索增强
- 🧠 长期记忆：基于对话上下文自动更新
- 🎙️ 语音：语音输入、文本转语音、语音转文本、流式音频播放
...
语音能力通过 WebSocket 服务对接（TTS/STT），需在 `backend/.env` 配置：
...
- `POST /api/friend/message/stt/stt/` (语音转文本)
