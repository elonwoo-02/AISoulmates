...
function handleTouchEnd() {
  ...
  if (Math.abs(currentX.value) > swipeThreshold) {
    ...
    const wasTap = Math.abs(currentX.value) < 10;
    // Snap back to center
    resetCard();
    // Trigger chat if it was a tap
    if (wasTap) {
      openChat();
    }
  }
...
<template>
  <div class="relative flex items-center justify-center w-full h-screen">
    ...
    <div
      ...
      @click="openChat"
    >
      <article class="overflow-hidden rounded-xl border border-black/5 bg-white shadow-[0_12px_30px_-20px_rgba(15,23,42,0.35)] h-full">
        <div class="relative h-[calc(100vh-120px)] overflow-hidden bg-[var(--surface-muted)]">
          <img
